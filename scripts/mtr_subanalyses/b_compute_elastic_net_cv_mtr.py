"""Evaluate predictive value of MTR imaging markers with elastic net regression.
...
This analysis uses a subsample with available MTR images and hence lives outside the main analysis
pipeline.

Requirements:
    - the data preparation scripts of the main analysis pipeline were run
...
CLINICAL_DATA_CLEANED_CSV = Path(__file__).parents[1] / "b_fetch_clinical_data.csv"

# %%
csv_files = list(PREPARED_DATA_DIR_MTR.glob("*.csv"))
clinical_data_df = pd.read_csv(CLINICAL_DATA_CLEANED_CSV, sep=";")
PREDICTION_OUTPUT_DIR_MTR.mkdir(parents=True, exist_ok=True)

# MTR does not contain all session of the clinical data df. Drop here to allow simple reuse of main
# analysis code
pass
...
for csv in csv_files:
    ...
    output_name = PREDICTION_OUTPUT_DIR_MTR / f"{marker_set_name}_elastic_net.csv"
    ...
    full_data_df = brain_marker_df.merge(
        ...
        how="left",  # left join as some sessions are missing for MTR
        validate="one_to_one",
    )
